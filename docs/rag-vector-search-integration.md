...
按方案完成全部代码改动，使用 Chroma(Docker) + DashScope text-embedding-v3 + 混合检索，未执行依赖下载（用户后续执行 `npm --prefix server install` + `docker compose up -d chroma` 即可启用）。

## 文件清单

### 新增文件

- [docker-compose.yml](file:///c:/Users/30565/Desktop/worker/within-the-enterprise/docker-compose.yml) — Chroma 容器定义，持久化到 `.runtime/chroma`
- [server/src/services/embeddingService.js](file:///c:/Users/30565/Desktop/worker/within-the-enterprise/server/src/services/embeddingService.js) — 向量库服务（切片/upsert/delete/semanticSearch/initVectorStore）
- [server/scripts/reindex.js](file:///c:/Users/30565/Desktop/worker/within-the-enterprise/server/scripts/reindex.js) — 批量重建脚本

### 修改文件

- [server/package.json](file:///c:/Users/30565/Desktop/worker/within-the-enterprise/server/package.json) — 加 `@langchain/community` + `chromadb` 依赖、`reindex` script
...
# 2. 启动 Chroma（Docker 必须已装）
docker compose up -d chroma
# 验证：curl http://localhost:8000/api/v2/heartbeat

# 3. 配置 server/.env（参考 .env.example，密钥复用 LLM_API_KEY）
#    RAG_ENABLED=true
#    CHROMA_URL=http://localhost:8000

# 4. 启动后端（自动初始化向量库 + 灌种子数据）
...
- Chroma 容器未起 → initVectorStore 失败仅记日志，不阻断服务；semanticSearch 返回空，searchDocumentsHybrid 自动回退关键词
...
- **局限性与建议**：
  ...
  - 可扩展性：可在 `embeddingService` 中添加“驱动层”（env 切换）以支持托管/本地向量 DB，后续我可帮助实现该切换器。
