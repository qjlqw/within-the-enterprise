...
- **当前结论**：A 级任务“Upstash Vector adapter + embeddingService 接入”已完成并已通过端到端测试；B 级任务“Redis-backed 队列（BullMQ）”已完成代码层接入，但真实生产环境验证仍需在 Node 20.12+ + Upstash Redis 凭证下实际运行,暂时跳过，在所有任务执行完之后统一验证；C 级任务“Backblaze B2 + 托管 DB”还未真正落地，因为仍缺 R2 / 托管 DB 凭证和具体接口接入。
- **未做/未验证**：没有迁移现有已发布文档，也没有把本地现有文档重新上传到 Upstash；没有接入 Backblaze B2 上传和托管 DB；没有在生产环境中用真实 Upstash Redis 验证 BullMQ worker。
...
### B. `indexQueue` 改成 Redis-backed + BullMQ（已编写，需在真实环境验证）
...
- **当前状态**：
  - 代码层已接通。
  - 但在当前工作区/当前 Node runtime 中，`bullmq` 与 `ioredis` 的真实运行时验证尚未在 Upstash Redis 真凭证下完成。

### C. Backblaze B2 + 托管 DB 接口（未真正落地）

- **当前状态**：
  - R2 对象存储尚未接入；
  - 托管数据库（如 Supabase / Postgres）元数据接口尚未切换；
  - `db` 接口仍保持现有实现，不存在真正的托管数据库连接和迁移逻辑。
  - 此项需要真实域名 / token / bucket 方案，当前未提供。
...
- **踩坑记录**：
  ...
  3. **Node 版本不一致**：项目 `package.json` 声明 `engines.node >=22.12.0 <23`，但当前实际环境是 `v20.18.0`，导致安装/解析 `bullmq` / `ioredis` 时出现 `EBADENGINE` 警告和可解析问题。
...
- 关键参数与版本：
  ...
  - `bullmq` 2.x
  - `ioredis` 5.x
  - Node 需 >=22.12（推荐），否则安装/运行更容易出现兼容问题。
...
- [ ] 真实 Upstash Redis + BullMQ worker 生产环境验证（需要 Node 20.12+ + 凭证），暂时跳过，在所有任务执行完之后统一验证
- [ ] Backblaze B2 文件对象存储接入
- [ ] 托管 DB 接口切换与元数据持久化
- [ ] 迁移已有文档索引 / 数据迁移
- [ ] 真实 document upload flow + source registry 同步
...
2. **配置真实 env**：
   ...
   - `UPSTASH_VECTOR_URL` / `UPSTASH_VECTOR_API_KEY`
   - `UPSTASH_REDIS_URL` / `UPSTASH_REDIS_TOKEN`
3. **运行真实队列验证**：在真实服务重启后，调用 `enqueueIndexJob`，确认 BullMQ worker 从 Upstash Redis 拉取 job 并处理成功。
4. **Backblaze B2 + 托管 DB**：只在拿到 R2 bucket、DB host / username / password / URL 之后再接入，不要在没有凭证时硬编码。
5. **明确迁移边界**：当前要求仅新上传文档写入新架构，不处理历史已发布文档；历史文档可以留在本地或后续单独迁移脚本执行。

**6. 风险 / 注意事项**

- **Node 版本风险**：当前本地 runtime 为 `v20.18.0`，但项目要求 `>=22.12`。直接在这个版本上运行会影响依赖安装与生产兼容性。
- **依赖解析风险**：`bullmq` / `ioredis` 若只放在 `optionalDependencies`，部署环境可能不安装，导致 `Cannot find package 'bullmq'`。
- **R2 / DB 凭证风险**：没有托管 DB 和 Backblaze B2 凭证之前，C 项目不能真正落地。不要在公共仓库里直接放 token / password。
...
1. 切到 Node 22.12+ 环境，安装 `bullmq` 与 `ioredis`。
...
- indexQueue 已支持 Redis-backed BullMQ 路径（server/src/services/indexQueue.js），但真实生产环境尚未在 Upstash Redis 凭证下运行验证。
- 当前明确要求：不迁移现有已发布文档，不重建当前的本地文档，不强制把历史数据重新上传至新架构；优先保证新文档写入新架构链路可用。
- 仍未完成：Backblaze B2 上传接入、托管 DB 接口切换。
...
3. 启动服务，执行 `enqueueIndexJob`，确认 `BullMQ` 从 Upstash Redis 拉取 job 并处理成功。
4. 收到 Backblaze B2 与托管 DB 凭证后，补齐上传存储和 `db` 接口，确保新上传文档写入 R2 + metadata DB + vector index。
